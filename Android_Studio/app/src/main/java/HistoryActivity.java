...
public class HistoryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Uri fileUri = Uri.parse(getIntent().getStringExtra("selectedFileUri")); // 선택된 txt 파일 전달 받기
        ...
        TextView textView2 = findViewById(R.id.linechart_title);
        TextView textview3 = findViewById(R.id.barchart_title);
        LineChart lineChart = findViewById(R.id.line_chart);
        HorizontalBarChart barChart = findViewById(R.id.bar_chart);
        LinearLayout chartBorderLayout = findViewById(R.id.border_chart_layout);
        LinearLayout chartBorderLayout2 = findViewById(R.id.border_chart_layout2);
        ...
        Map<String, List<HistoryActivity.FrameData>> frameData = new HashMap<>();
        String maxClass = "";
        String videoPath = "";
        long frameTime = 0;

        // 파일 내용을 읽기 위한 InputStream 준비
        if (fileUri != null) {
            try {
                ...
                while ((line = br.readLine()) != null) { // txt 파일의 각 줄 읽기
                    ...
                    frameData.get(classLabel).add(new HistoryActivity.FrameData(video, frame));
            ...
            chartBorderLayout.setVisibility(View.GONE);
            chartBorderLayout2.setVisibility(View.GONE);
            Toast.makeText(this, "No selected file.", Toast.LENGTH_LONG).show();
        }

        Random random = new Random();
        List<HistoryActivity.FrameData> maxClassFrames = frameData.get(maxClass);
        HistoryActivity.FrameData selectedFrame = maxClassFrames.get(random.nextInt(maxClassFrames.size()));
        ...
        String maxClassDescription = classDescriptions.getOrDefault(maxClass, "No description available");
        String baseText = "가장 오래 측정된 수면 자세는 \n";
        String endText = "입니다!";
        SpannableStringBuilder spannableString = new SpannableStringBuilder(baseText + maxClassDescription + endText);
        // maxClassDescription 텍스트 설정
        int start = baseText.length();
        int end = start + maxClassDescription.length();
